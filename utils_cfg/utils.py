...
from utils_cfg.torch_utils import init_seeds, is_parallel
import cv2
import matplotlib
...
def compute_loss_v3(p, targets, model):  # predictions, targets, model
...
def compute_loss_scalev4(p, targets, model):  # predictions, targets, model
    device = targets.device
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors = build_targets_v4(p, targets, model)  # targets
    h = model.hyp  # hyperparameters
...
def build_targets_v4(p, targets, model):
...
def strip_optimizer(f='weights/last.pt'):  # from utils_cfg.utils_cfg import *; strip_optimizer()
...
def create_backbone(f='weights/last.pt'):  # from utils_cfg.utils_cfg import *; create_backbone()
...
def coco_only_people(path='../coco/labels/train2017/'):  # from utils_cfg.utils_cfg import *; coco_only_people()
...
def select_best_evolve(path='evolve*.txt'):  # from utils_cfg.utils_cfg import *; select_best_evolve()
...
def crop_images_random(path='../images/', scale=0.50):  # from utils_cfg.utils_cfg import *; crop_images_random()
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    # Makes single-class coco datasets. from utils_cfg.utils_cfg import *; coco_single_class_labels()
...
def kmean_anchors(path='./data/coco64.txt', n=9, img_size=(320, 1024), thr=0.20, gen=1000):
    # Creates kmeans anchors for use in *.cfg files: from utils_cfg.utils_cfg import *; _ = kmean_anchors()
    # n: number of anchors
    # img_size: (min, max) image size used for multi-scale training (can be same values)
    # thr: IoU threshold hyperparameter used for training (0.0 - 1.0)
    # gen: generations to evolve anchors using genetic algorithm
    from utils_cfg.datasets import LoadImagesAndLabels
...
def plot_wh_methods():  # from utils_cfg.utils_cfg import *; plot_wh_methods()
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if names:
                    if cls >= len(names) or cls < 0:
                        cls = cls
                        print("why number of class is %d (>= %d)"%(cls, len(names)))
                    else:
                        cls = len(names)
                else:
                    cls = cls
...
def plot_test_txt():  # from utils_cfg.utils_cfg import *; plot_test()
...
def plot_targets_txt():  # from utils_cfg.utils_cfg import *; plot_targets_txt()
...
def plot_evolution_results(hyp):  # from utils_cfg.utils_cfg import *; plot_evolution_results(hyp)
...
def plot_results_overlay(start=0, stop=0):  # from utils_cfg.utils_cfg import *; plot_results_overlay()
...
def plot_results(start=0, stop=0, bucket='', id=()):  # from utils_cfg.utils_cfg import *; plot_results()
